export default function CarouselSliderSmall({ swiper, on }) {
    on('beforeInit', () => {
    ...
    on('progress', () => {
        ...
        for (let i = 0; i < swiper.slides.length; i += 1) {
            ...
            if (zIndex === 39) {
                slideEl.style.transform = `translate(${translate}, -44px) scale(${scale})`
            } else slideEl.style.transform = `translateX(${translate}) scale(${scale})`;
            if (absProgress > 3) {
